...
/* ---------- Improve bullets ---------- */
export async function improveBullets(input: { bullets: string[]; role?: string; company?: string }) {
  ...
  const result = await callAI({
    messages: [
      ...
      {
        role: 'user',
        content: `Rewrite these bullets${input.role ? ` for a ${input.role}` : ''}${input.company ? ` at ${input.company}` : ''}:\n\n${input.bullets.map((b, i) => `${i + 1}. ${b || '(empty)'}`).join('\n')}`,
      },
    ],
    tools: [
      {
...
/* ---------- ATS score ---------- */
export async function analyzeATS(input: { resumeText: string; jobDescription?: string }) {
  ...
  const result = await callAI({
    messages: [
      ...
      {
        role: 'user',
        content: `Analyze this resume for ATS compatibility${input.jobDescription ? ' against the provided job description' : ''}.\n\n=== RESUME ===\n${input.resumeText}${input.jobDescription ? `\n\n=== JOB DESCRIPTION ===\n${input.jobDescription}` : ''}`,
      },
    ],
    tools: [
      {
